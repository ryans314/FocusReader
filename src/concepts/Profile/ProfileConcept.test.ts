...
Deno.test("Profile Concept Tests Suite", {
  ...
  await test.step("Action: createAccount - Successful creation", async () => {
    ...
    console.log(
      `  Attempted to create account for ${username}. Result: ${
    ...
    assertExists(
      ...
      "User ID should be returned on successful creation.",
    );
    ...
    assertEquals(
      users[0].username,
      username,
      "The created user's username should match.",
    );

    const userDetails = await profileConcept._getUserDetails({
      user: (result as { user: ID }).user,
    });
  ...
  await test.step("Action: createAccount - Duplicate username", async () => {
    ...
    console.log(
      `  Attempted duplicate account for ${username}. Result: ${
        JSON.stringify(result)
      }`,
    );
    ...
    assertExists(
      ...
      "An error should be returned for duplicate username.",
    );
    assertEquals(
      (result as { error: string }).error,
  ...
  await test.step("Action: createAccount - Insecure password", async () => {
    ...
    const result = await profileConcept.createAccount({ username, password });
    console.log(
      `  Attempted to create account for ${username} with insecure password. Result: ${
        JSON.stringify(result)
      }`,
    );
    ...
    assertExists(
      (result as { error: string }).error,
      "An error should be returned for insecure password.",
    );
    assertEquals(
      (result as { error: string }).error,
  ...
  await test.step("Action: authenticate - Successful authentication", async () => {
    ...
    const { user: userId } =
      (await profileConcept.createAccount({ username, password })) as {
        user: ID;
      };
    ...
    console.log(
      `  Attempted to authenticate ${username}. Result: ${
        JSON.stringify(result)
      }`,
    );
    ...
    assertExists(
      (result as { user: ID }).user,
      "User ID should be returned on successful authentication.",
    );
    assertEquals(
      (result as { user: ID }).user,
      userId,
  ...
  await test.step("Action: authenticate - Invalid username", async () => {
    ...
    const result = await profileConcept.authenticate({ username, password });
    console.log(
      `  Attempted to authenticate non-existent user ${username}. Result: ${
        JSON.stringify(result)
      }`,
    );
    ...
    assertExists(
      (result as { error: string }).error,
      "An error should be returned for invalid username.",
    );
    assertEquals(
      (result as { error: string }).error,
  ...
  await test.step("Action: authenticate - Incorrect password", async () => {
    ...
    const result = await profileConcept.authenticate({
      username,
      password: "wrongPassword",
    });
    console.log(
      `  Attempted to authenticate ${username} with wrong password. Result: ${
        JSON.stringify(result)
      }`,
    );
    ...
    assertExists(
      (result as { error: string }).error,
      "An error should be returned for incorrect password.",
    );
    assertEquals(
      (result as { error: string }).error,
      "Invalid username or password.",
  ...
  await test.step("Action: changePassword - Successful password change", async () => {
    ...
    const { user: userId } =
      (await profileConcept.createAccount({
        username,
        password: oldPassword,
      })) as { user: ID };
    console.log(`  Created account for ${username} with ID ${userId}.`);

    const authOld = await profileConcept.authenticate({
      username,
      password: oldPassword,
    });
    assertExists(
      (authOld as { user: ID }).user,
      "Should authenticate with old password successfully.",
    );
    console.log(`  Authenticated with old password.`);

    const result = await profileConcept.changePassword({
      user: userId,
      oldPassword,
      newPassword,
    });
    console.log(
      `  Attempted to change password for ${username}. Result: ${
        JSON.stringify(result)
      }`,
    );
    ...
    assertExists(
      (result as { user: ID }).user,
      "User ID should be returned on successful password change.",
    );
    assertEquals(
      (result as { user: ID }).user,
      userId,
    ...
    const authFailed = await profileConcept.authenticate({
      username,
      password: oldPassword,
    });
    assertExists(
      (authFailed as { error: string }).error,
      "Old password should no longer work.",
    );
    console.log(
      `  Attempt to authenticate with old password (expected failure): ${
        JSON.stringify(authFailed)
      }`,
    );

    const authSuccess = await profileConcept.authenticate({
      username,
      password: newPassword,
    });
    assertExists(
      (authSuccess as { user: ID }).user,
      "New password should work.",
    );
    assertEquals(
      (authSuccess as { user: ID }).user,
      userId,
      "Authenticated with new password successfully.",
    );
  ...
  await test.step("Action: changePassword - Incorrect old password", async () => {
    ...
    console.log(
      `  Attempted to change password for ${username} with incorrect old password. Result: ${
        JSON.stringify(result)
      }`,
    );
    ...
    assertExists(
      (result as { error: string }).error,
      "An error should be returned for incorrect old password.",
    );
    assertEquals(
      (result as { error: string }).error,
  ...
  await test.step("Action: changePassword - Insecure new password", async () => {
    ...
    console.log(
      `  Attempted to change password for ${username} with insecure new password. Result: ${
        JSON.stringify(result)
      }`,
    );
    ...
    assertExists(
      (result as { error: string }).error,
      "An error should be returned for insecure new password.",
    );
    assertEquals(
      (result as { error: string }).error,
  ...
  await test.step("Action: changePassword - New password same as old", async () => {
    ...
    console.log(
      `  Attempted to change password for ${username} to the same password. Result: ${
        JSON.stringify(result)
      }`,
    );
    ...
    assertExists(
      (result as { error: string }).error,
      "An error should be returned when new password is same as old.",
    );
    assertEquals(
      (result as { error: string }).error,
  ...
  await test.step("Action: changePassword - User not found", async () => {
    ...
    console.log(
      `  Attempted to change password for non-existent user. Result: ${
        JSON.stringify(result)
      }`,
    );
    ...
    assertExists(
      (result as { error: string }).error,
      "An error should be returned for a non-existent user.",
    );
    assertEquals(
      (result as { error: string }).error,
  ...
  await test.step("Action: deleteAccount - Successful deletion", async () => {
    ...
    console.log(
      `  Attempted to delete account for ${username}. Result: ${
        JSON.stringify(result)
      }`,
    );
    ...
    console.log(
      `  Attempted to authenticate deleted user (expected failure): ${
  ...
  await test.step("Action: deleteAccount - User not found", async () => {
    ...
    const result = await profileConcept.deleteAccount({
      user: nonExistentUser,
    });
    console.log(
      `  Attempted to delete non-existent user. Result: ${
        JSON.stringify(result)
      }`,
    );
  ...
  await test.step("Query: _getUserDetails - Retrieve user details", async () => {
    ...
    console.log(
      `  Retrieved details for user ${userId}: ${JSON.stringify(userDetails)}`,
    );
    ...
    assertEquals(
      Object.keys((userDetails as Array<{ username: string }>)[0]).includes(
        "passwordHash",
      ),
      false,
      "Password hash should NOT be returned by _getUserDetails.",
    );
  });
  ...
  await test.step("Query: _getUserDetails - User not found", async () => {
    ...
    const result = await profileConcept._getUserDetails({
      user: nonExistentUser,
    });
    console.log(
      `  Attempted to get details for non-existent user. Result: ${
        JSON.stringify(result)
      }`,
    );
  ...
  await test.step("Query: _getAllUsers - Retrieve all users", async () => {
    ...
    assertEquals(
      usernames,
      ["user1", "user2"],
      "Retrieved usernames should match.",
    );
  });
  ...
  await test.step("Principle: A user can create an account and sign in with a username/password. Users can also change their password.", async () => {
    ...
    console.log(
      `1. Action: createAccount('${username}', '${initialPassword}')`,
    );
    const createResult = await profileConcept.createAccount({
      username,
      password: initialPassword,
    });
    assertExists(
      (createResult as { user: ID }).user,
    ...
    const authInitialResult = await profileConcept.authenticate({
      username,
      password: initialPassword,
    });
    assertExists(
      (authInitialResult as { user: ID }).user,
    ...
    console.log(
      `   Effect: User '${username}' successfully authenticated with initial password.`,
    );

    console.log(
      `3. Action: changePassword('${userId}', '${initialPassword}', '${newPassword}')`,
    );
    ...
    assertExists(
      (changePasswordResult as { user: ID }).user,
      "Password change should be successful.",
    );
    assertEquals(
      (changePasswordResult as { user: ID }).user,
      userId,
      "Changed password for correct user ID.",
    );
    console.log(
      `   Effect: User '${username}' password changed from initial to new password.`,
    );

    console.log(
      `4. Action: authenticate('${username}', '${initialPassword}') (Expected failure)`,
    );
    const authOldPasswordResult = await profileConcept.authenticate({
      username,
      password: initialPassword,
    });
    assertExists(
      (authOldPasswordResult as { error: string }).error,
      "Authentication with old password should fail.",
    );
    console.log(
      `   Effect: Authentication with old password failed as expected: ${
        JSON.stringify((authOldPasswordResult as { error: string }).error)
      }`,
    );

    console.log(
      `5. Action: authenticate('${username}', '${newPassword}') (Expected success)`,
    );
    const authNewPasswordResult = await profileConcept.authenticate({
      username,
      password: newPassword,
    });
    assertExists(
      (authNewPasswordResult as { user: ID }).user,
    ...
    console.log(
      `   Effect: User '${username}' successfully authenticated with new password.`,
    );
