...
content_id: ef43304f63c13bbe28b38d56e248e84fbc105c3be1598df750133963b0ced775
---

# Modified Concept Implementation
...
// Generic types of this concept (Font removed)
type User = ID;
type Document = ID;
type TextSettings = ID; // TextSettings itself is still an ID

/**
 * a set of TextSettings with:
 * a font String // Changed type here
 * a fontSize number
 * a lineHeight number
 */
interface TextSettingsDoc {
  _id: TextSettings;
  font: string; // Changed type here
  fontSize: number;
  lineHeight: number;
}
...
export default class TextSettingsConcept {
  ...
  async createSettings(
    ...
    // Basic validation for font string
    if (!font || font.trim() === "") {
   ...
   */
  async _getTextSettingsProperties(
    { settingsId }: { settingsId: TextSettings }, // Renamed argument for clarity
  ): Promise<
    Array<{ font: string; fontSize: number; lineHeight: number }> | { // Changed font type here
      error: string;
    }
  > {
   ...
   */
  async _getDefaultTextSettingsIdForUser(
    { user }: { user: User },
   ...
   */
  async _getCurrentTextSettingsIdForDocument(
