...
Deno.test("FocusStats Concept Tests", async (t) => {
  ...
  await t.step(
    ...
    async () => {
      ...
      assertExists(result);
      // Check for error property, if it exists, the test would fail unless it's an expected error.
      assertNotEquals(
        (result as { error: string }).error,
        "User Alice already has FocusStats initialized.", // This is the error we expect for duplicates
        "Expected initUser to succeed, but it returned a specific error.",
      );

      const { focusStats: userAFocusStatsId } = result as { focusStats: ID };
      console.log(`initUser(${userA}): FocusStats ID = ${userAFocusStatsId}`);

      // Verify state via _viewStats query
  ...
  let session1Id: ID; // Declare here to be accessible in later tests
  await t.step(
    ...
    async () => {
      ...
      const result = await focusStatsConcept.startSession({
        user: userA,
        document: doc1,
        library: libraryA, // Assuming external library check passes
      });
      assertExists(result);
      assertNotEquals(
        (result as { error: string }).error,
        "User does not have FocusStats initialized.",
        "Expected startSession to succeed.",
      );

      ({ focusSession: session1Id } = result as { focusSession: ID });
      console.log(
  ...
  let session2Id: ID; // Declare here to be accessible in later tests
  const sessionResult = await focusStatsConcept.startSession({
    user: userA,
    document: doc2,
    library: libraryA,
  });
  session2Id = (sessionResult as { focusSession: ID }).focusSession;
  console.log(
  ...
  await t.step(
    ...
    async () => {
      ...
      assertNotEquals(
        (result as { error: string }).error,
        "FocusSession not found.", // A different error message would indicate failure here
        "Expected endSession to succeed.",
      );
  ...
  session3Id = (session3Result as { focusSession: ID }).focusSession;
  ...
  await t.step(
    ...
    async () => {
      ...
      assertNotEquals(
        ...
        "Expected removeSession to succeed.",
      );
  ...
  await t.step(
    ...
    async () => {
      ...
      assertNotEquals(
        ...
        "Expected _viewStats to succeed.",
      );
      ...
      // Note: session1Id was started but never ended, so it shouldn't be in focusSessionIds
      assertEquals(stats.focusSessionIds.includes(session1Id), false);
      console.log(
  ...
  await t.step(
    ...
    async () => {
      ...
      assertNotEquals(
        (result as { error: string }).error,
        "FocusStats not found for user.",
        "Expected _getSessions to succeed.",
      );
      ...
      assertEquals(
        sessions.some((s) => s.focusSession._id === session1Id),
        true,
      );
      assertEquals(
        sessions.some((s) =>
      ...
      console.log(
        `_getSessions(${userA}): Retrieved sessions: ${
          JSON.stringify(sessions.map((s) => s.focusSession._id))
        } ✅`,
      );
    },
  );
  ...
  await t.step(
    ...
    async () => {
      ...
      console.log(`   Updated stats verified: both sessions included. ✅`);
